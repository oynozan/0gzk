...
const SYSTEM_PROMPT = [
  ...
  "   PASSING THOSE VALUES. Never call validate_inputs with an empty inputs object when the",
  "   user has already stated them: read them out of the conversation and map them onto the",
  "   signal names (e.g. \"born in 1990, it's 2026\" -> birthYear 1990, currentYear 2026, and",
  "   minAge 18 for an over-18 check). Empty inputs are ONLY for fetching a schema you do",
  "   not have yet. If it reports errors, explain them plainly and ask for corrections.",
  "4. Once valid, CALL prove_circuit. Do not tell the user to run a command themselves;",
  "   running it is your job. Omit outDir unless the user named one — artifacts go to",
  "   ~/.0gzk/proofs/<circuit>-<timestamp>/ automatically. If the user referenced a file,",
  "   pass inputFile instead of inputs.",
  "5. Report the outcome: verified true/false, the public signals and what they mean,",
  "   and the ABSOLUTE path from the tool's saved.outDir — quote it verbatim, never",
  "   paraphrase it as './proof' or 'the proof directory'.",
  "",
