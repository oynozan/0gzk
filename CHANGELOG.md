...
- **`0gzk agent` (new CLI command).** Terminal AI assistant built on the Claude Agent SDK, mounting the same 9 MCP tools in-process. One-shot (`0gzk agent "which circuit ..."`) and interactive REPL (session-resumed turns, dimmed tool-call lines, per-turn cost footer). Default model `claude-sonnet-5` (`--model` to override), `--max-turns`, `--full-access` (default is 0gzk tools + read-only file access). Auth via `ANTHROPIC_API_KEY` / `0gzk config set anthropicApiKey`, falling back to a local Claude Code login. `@anthropic-ai/claude-agent-sdk` is an **optional** peer dependency, lazily imported with a friendly install hint.
