...
**BREAKING (behavior): the default network is now `base` (chain ID 8453), not `0g-mainnet`.** Anything that relied on the implicit 0G default now talks to Base. What moves with it:

- [`packages/sdk/src/node/config.ts`](./packages/sdk/src/node/config.ts): `loadConfig()` with nothing set resolves `network: "base"`, `chainId: 8453`, `rpcUrl: https://mainnet.base.org`, `explorer: https://basescan.org`. Because Base is `family: "evm"`, **`storage` defaults to `"ipfs"`** (Pinata-style pinning API + public gateway) instead of 0G Storage — so out of the box publishing needs a pinning token rather than a funded 0G wallet, and fetching needs no credential at all.
- [`packages/sdk/src/onchain/index.ts`](./packages/sdk/src/onchain/index.ts): `getRegistryContract(runner, address?, chainId = 8453)` — the `chainId` default moved from `16661` to `8453`.
- [`packages/mcp/src/tools/discovery.ts`](./packages/mcp/src/tools/discovery.ts) and [`tools/authoring.ts`](./packages/mcp/src/tools/authoring.ts): the default `chain` for every registry-backed tool is `base`.
- [`web/lib/server/registry.ts`](./web/lib/server/registry.ts) and [`web/lib/explorer.ts`](./web/lib/explorer.ts): the web app's server-side registry reads and explorer links default to `base`.
- **Switching back to 0G** is one setting, and it takes the storage backend with it: `0gzk config set network 0g-mainnet`, `--network 0g-mainnet` on a single command, `OGZK_NETWORK=0g-mainnet` in the environment, or `loadConfig({ network: "0g-mainnet" })` programmatically.
- The registry is deployed on **all four chains** and baked into `REGISTRY_ADDRESSES`: `0xCe9f0DF51abeC7B8cD751067c6D8d3db5E2bE64d` on Base mainnet (8453), Base Sepolia (84532) and 0G mainnet (16661); `0x5b2c3e86c9255a4459199a6d9cb7b63e2a660ce6` on 0G Galileo testnet (16602). All 14 reference circuits are published on Base mainnet and Base Sepolia with bundles pinned to IPFS; the earlier 0G publications remain live with bundles on 0G Storage.

**BREAKING (API): `POST /api/agent` response shape.** It now returns `{ done, reply, trace, model }` and, when the model asked for a tool the server must not run, `{ done: false, clientToolCalls, messages }`. Clients must loop until `done: true` instead of reading `reply` from the first response. The request body accepts `user`, `assistant` (including `tool_calls`) and `tool` turns, and must end with a user turn or client-side tool results.

### Added

- **Multi-chain network model.** [`packages/sdk/src/networks.ts`](./packages/sdk/src/networks.ts) — a single isomorphic preset table shared by SDK, CLI, web, and examples, exported from the `@0gzk/sdk` root and re-exported from `/node`: `NETWORKS`, `resolveNetwork`, `networkForChainId`, `explorerTxUrl`, `explorerAddressUrl`. Canonical names: `base` (8453, **the default**), `base-sepolia` (84532), `0g-mainnet` (16661), `0g-testnet` (16602); Base presets carry public RPC + basescan explorer config.
- **Pluggable storage backends.** `uploadBundle`/`fetchBundle` keep their signatures but route through a `StorageBackend` interface ([`packages/sdk/src/node/storage/`](./packages/sdk/src/node/storage)) with two implementations:
  ...
  - `ipfs` — NEW: uploads via any `pinFileToIPFS`-compatible pinning API (default `https://api.pinata.cloud/pinning/pinFileToIPFS`, Bearer token required, pins as **CIDv0**) and fetches via HTTP gateways with automatic fallback (configured gateway first, then Pinata / ipfs.io / dweb.link / w3s.link; default `https://gateway.pinata.cloud`). No wallet, no gas.
...
- **CLI additions** ([`packages/cli`](./packages/cli)):
  ...
  - `0gzk config` gains keys: `storage`, `storageNetwork`, `ipfsApiUrl`, `ipfsApiToken` (secret, masked), `ipfsGateway`, `anthropicApiKey` (secret, masked; used by `0gzk agent --local`), `agentUrl` (hosted `0gzk agent` endpoint; default `https://0gzk.com/api/agent`).
...
- **`@0gzk/mcp` (new package,** [`packages/mcp`](./packages/mcp)**).** An MCP server for circuit discovery, proving, and authoring — **11 tools in repo mode, 8 in discovery mode**: 5 discovery tools (`search_circuits`, `list_circuits`, `get_circuit`, `get_example_input`, `resolve_circuit` — chain-aware, catalog-backed in a repo checkout, live-registry-backed anywhere), 2 input tools (`validate_inputs`, `read_input_file`), and 4 authoring tools (`scaffold_circuit`, `validate_metadata`, `build_circuit`, `prove_circuit` — the last available in both modes), plus `ogzk://guide/circom-authoring` and `ogzk://catalog` resources. Ships a `0gzk-mcp` stdio binary; the repo carries a pre-wired [`.mcp.json`](./.mcp.json) for Claude Code. Built on `@modelcontextprotocol/server` v2.
- **Input tools + proof artifacts** ([`packages/mcp/src/tools/inputs.ts`](./packages/mcp/src/tools/inputs.ts), [`tools/authoring.ts`](./packages/mcp/src/tools/authoring.ts)) — what turns an assistant from "here's the command to run" into "here's your proof":
  - `validate_inputs` checks candidate values against a circuit's declared signal schema **without proving**, returning `{valid, errors, schema}`. Errors name the offending signal and its expected type and never echo the user's values. Called with no `inputs` it returns the schema plus `missing`, which is how an agent learns what to ask for; catalog circuits answer instantly with no bundle download.
  - `read_input_file` reads a local JSON input file and reports **only signal names and value types** — the values stay on disk. Pass the same path to `prove_circuit` as `inputFile`.
  - `prove_circuit` gains `inputFile` (read inputs from a local JSON path instead of inline `inputs`, keeping them off the wire) and `outDir` (writes `proof.json`, `public.json`, `result.json`, creating the directory if missing).
- **`0gzk agent` (new CLI command) — the agent does the job, it does not hand out instructions.** Terminal AI assistant with a Claude Code-style interface (welcome banner, streamed output, `⏺ tool(args)` / `⎿ result` trace lines, per-turn footer). One-shot (`0gzk agent "..."`) and interactive chat. Its flow is: find the circuit → show the required signals and ask for missing values → `validate_inputs` → `prove_circuit` with an `outDir` (default `./proof`) → report `verified`, the public signals and where the files landed. Verified end-to-end: `0gzk agent "Prove that I am over 18. I was born in 1990, the current year is 2026. Save the proof to /tmp/agent-proof"` produces real Groth16 artifacts with `publicSignals ["1","2026","18"]` and `verified: true`, with `birthYear` never leaving the machine. Two modes:
  - **Default — hosted, no API key.** The conversation goes to the 0gzk web deployment's new `POST /api/agent` route ([`web/app/api/agent`](./web/app/api/agent)), which runs `gpt-5-nano` (server-side `OPENAI_API_KEY`; `OGZK_AGENT_MODEL`/`OPENAI_BASE_URL` overridable for OpenAI-compatible providers) as a real MCP client over the 5 read-only discovery tools plus the bundled circuit catalog. CLI endpoint override: `0gzk config set agentUrl <url>` / `OGZK_AGENT_URL`.
  - **Client-side tool execution (the privacy mechanic).** `validate_inputs`, `read_input_file` and `prove_circuit` ([`CLIENT_TOOL_NAMES`](./packages/mcp/src/tools/index.ts)) are **declared** to the model server-side but **never executed** there. When the model calls one, `/api/agent` returns `{done:false, clientToolCalls, messages}`; the CLI runs it locally ([`packages/cli/src/agent/client-tools.ts`](./packages/cli/src/agent/client-tools.ts), rendered as `· local` in the trace), appends `{role:"tool", tool_call_id, content}` and posts the conversation back — repeating until `{done:true, reply}` ([`packages/cli/src/agent/remote.ts`](./packages/cli/src/agent/remote.ts)). The witness is built and consumed on the user's machine; only tool summaries (capped at 8,000 chars), the proof, and the public signals travel. The endpoint stays **stateless**: the server echoes `messages` and the CLI replays them. Discovery tools still run server-side over MCP — they involve no private data — and the authoring tools are never exposed publicly at all.
  - **`--local`** — Claude Agent SDK in-process with the full toolset including authoring (scaffold → build → prove): 11 tools inside a repo checkout, 8 outside one, plus read-only file access. `--model` (default `claude-sonnet-5`), `--max-turns`, `--full-access`, `--repo-root`. Auth via `ANTHROPIC_API_KEY` / `0gzk config set anthropicApiKey`, falling back to a local Claude Code login. `@anthropic-ai/claude-agent-sdk` is an **optional** peer dependency, lazily imported with a friendly install hint.
- **Web `/ai` page** — agent & MCP guide with copyable install commands (`npm i -g @0gzk/cli`, `npm i @0gzk/sdk snarkjs`, `claude mcp add 0gzk -- npx -y @0gzk/mcp`), the MCP tool reference, and a sample session; the landing page gains an INSTALL block and the header an AI link.
...
- **Docs rewritten for both changes.** [`docs/content/agent.mdx`](./docs/content/agent.mdx) now leads with the agent running the job: an annotated session transcript with the local-execution trace, the client/server split and its privacy guarantee, a table of which tool runs where, the artifacts written, and the updated hosted-backend and `--local` sections. [`docs/content/mcp.mdx`](./docs/content/mcp.mdx) documents 11/8 tools with read-only vs filesystem effects. `networks`, `getting-started`, `cli`, `sdk/node`, `sdk/onchain`, `contracts`, `reference`, `index`, the root README, `packages/cli/README.md`, `packages/mcp/README.md` and `packages/sdk/USAGE.md` all state `base` as the default network (and `ipfs` as the default backend), with the four live registry addresses and the way back to 0G.
