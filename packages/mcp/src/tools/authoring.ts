...
export const proveCircuitTool = defineTool({
  name: "prove_circuit",
  description:
    "Generate a Groth16 proof (and locally verify it) for a circuit. Bundle source: a repo circuit's circuit_bundle/ by name " +
    "(falling back to the published registry bundle), an explicit bundleDir, or a published rootHash fetched into the cache. " +
    "Input validation errors describe the exact expected input schema.",
  readOnly: false,
  schema: {
    inputs: z.record(z.string(), z.unknown()).describe("Circuit inputs keyed by signal name (field elements as decimal strings)"),
    ...
    chain: chainSchema.optional().describe("Registry/storage chain for fetches (default 0g-mainnet)"),
  ...
  handler: async (ctx, args) => {
    ...
    try {
      const result = await generateProof(bundle, args.inputs);
