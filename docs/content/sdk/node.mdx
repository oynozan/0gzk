...
| --- | --- | --- |
| **`network`** | **`OGZK_NETWORK`**, **`OG_NETWORK`** (canonical name or alias; unknown values **throw**) | **`base`** |
| **`rpcUrl`** | **`OGZK_RPC_URL`**, **`OG_RPC_URL`** | preset RPC for **`network`** (**`https://mainnet.base.org`**) |
| **`indexerUrl`** | **`OG_INDEXER_URL`** | preset indexer for **`storageNetwork`** |
| **`privateKey`** | **`OGZK_PRIVATE_KEY`**, **`OG_PRIVATE_KEY`** | **`undefined`** |
| **`storage`** | **`OGZK_STORAGE`** (**`0g`** or **`ipfs`**) | **`0g`** on 0G chains, **`ipfs`** on other chains — so **`ipfs`** by default |
...
| **`ipfs.gateway`** | **`OGZK_IPFS_GATEWAY`** | **`https://gateway.pinata.cloud`** |
| **`chainId`** | (not read from env) | preset **`chainId`** (**`8453`**) |
| **`explorer`** | (not read from env) | preset **`explorer`** (**`https://basescan.org`**) |

**Breaking in 0.4.0:** the default network is **`base`**, not `0g-mainnet`. Since Base is `family: "evm"`, `storage` defaults to **`ipfs`** with it — a bare `loadConfig()` resolves the Base registry and the IPFS backend, and never touches a 0G endpoint. Code that relied on the implicit 0G default must now say so explicitly: `loadConfig({ network: "0g-mainnet" })` or `OGZK_NETWORK=0g-mainnet`.

```typescript
import { loadConfig } from "@0gzk/sdk/node";

const config = loadConfig();                             // env-only: base + ipfs
const zeroG = loadConfig({ network: "0g-mainnet" });     // back to 0G Chain + 0G Storage
const testnet = loadConfig({ network: "0g-testnet" });   // force 0G Galileo
const sepolia = loadConfig({ network: "base-sepolia" }); // storage stays "ipfs"
...
| `ref` form | Backend |
| --- | --- |
| bare **`0x...`** root hash | **`config.storage`** — **`ipfs`** for a `loadConfig()` on the default Base network, **`0g`** when the field is absent from the object you pass (the pre-0.4.0 behavior) |
...
// IPFS-hosted bundle (no indexer involved; gateway defaults to https://gateway.pinata.cloud).
