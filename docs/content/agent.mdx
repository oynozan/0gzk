...
`prove_circuit` writes when it is given an `outDir` (the agent passes one on every proving run; **`./proof`** is its default). A relative path resolves against the repo root when the CLI is inside a checkout, otherwise the current directory. The directory is created if missing.
